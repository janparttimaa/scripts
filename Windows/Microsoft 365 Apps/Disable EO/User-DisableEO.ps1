<#
.SYNOPSIS
    Disables EO for the current user via registry policy.

.DESCRIPTION
    This PowerShell script configures the DisableEO registry value under the
    ...
    Recommended for Microsoft Purview Information Protection. This will turn off "Encryption-Only" option from Outlook (classic).
...
.EXAMPLE
    ...
    powershell.exe -ExecutionPolicy Bypass -File .\User-DisableEO.ps1
    ...
    %windir%\sysnative\WindowsPowerShell\v1.0\powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -File "User-DisableEO.ps1"
...
Try {
    ...
    If ($ActualValue -eq $ExpectedValue) {
        Write-Output "SUCCESS: $ValueName is set to $ActualValue (DisableEO enabled for current user)."
        Exit 0
    }
    Else {
